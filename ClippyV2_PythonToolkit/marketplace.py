from flask import Flask, session, request, redirect, send_from_directory, jsonify
import os
import json

app = Flask(__name__)
app.secret_key = "supersecure"  # Replace for production
PLUGIN_DIR = "plugins"
...
def upload():
    ...
    # Save registry
    entry = { "id": filename[:-3], "name": name, "author": author, "uploaded_by": session.get("user", "anon") }
    try:
